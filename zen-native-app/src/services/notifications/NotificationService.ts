...
class NotificationService {
  ...
  /**
   * Schedule test notifications every 30 minutes
   * 테스트용: 30분마다 알림을 보냅니다
   * @returns Notification ID or null if no permission
   * @note This is for testing purposes only
   */
  async scheduleTestReminder(): Promise<string | null> {
    if (!this.hasPermission) {
      console.log('No notification permission for test reminder');
      return null;
    }
    ...
    try {
      ...
      const existingNotifications = await Notifications.getAllScheduledNotificationsAsync();
      const existingTestReminders = existingNotifications.filter(
        notif => notif.content.data?.type === 'test_reminder'
      );
      
      for (const reminder of existingTestReminders) {
        await this.cancelNotification(reminder.identifier);
        console.log('Cancelled existing test reminder:', reminder.identifier);
      }

      // Schedule notification for every 30 minutes
      const notificationId = await Notifications.scheduleNotificationAsync({
        content: {
        ...
        trigger: {
          seconds: 1800, // 30 minutes = 1800 seconds
          repeats: true
        },
      });

      console.log('Test reminder scheduled (every 30 min):', notificationId);
      return notificationId;
    } catch (error) {
  ...
  /**
   * Cancel test reminder notifications
   * 테스트 알림을 취소합니다
   * @returns true if cancelled successfully, false otherwise
   */
  async cancelTestReminder(): Promise<boolean> {
    try {
      const existingNotifications = await Notifications.getAllScheduledNotificationsAsync();
      const testReminders = existingNotifications.filter(
        notif => notif.content.data?.type === 'test_reminder'
      );
      
      if (testReminders.length === 0) {
        console.log('No test reminder to cancel');
        return false;
      }

      for (const reminder of testReminders) {
        await this.cancelNotification(reminder.identifier);
        console.log('Test reminder cancelled:', reminder.identifier);
      }
      
      return true;
    } catch (error) {
      console.error('Failed to cancel test reminder:', error);
      return false;
    }
  }

  /**
   * Check if test reminder is currently scheduled
   * 테스트 알림이 현재 스케줄되어 있는지 확인합니다
   * @returns true if test reminder is scheduled, false otherwise
   */
  async isTestReminderScheduled(): Promise<boolean> {
    try {
      const existingNotifications = await Notifications.getAllScheduledNotificationsAsync();
      return existingNotifications.some(
        notif => notif.content.data?.type === 'test_reminder'
      ...
      console.error('Failed to check test reminder status:', error);
      return false;
    }
  }
